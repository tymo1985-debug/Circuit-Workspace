 ...
 *
 * Исключение, добавленное осознанно 31.08.2026: `1181` — не новый экранный
 * breakpoint, а парная граница к уже существующему `1180` (та же природа,
 * что у пар 767/768, 1100/1101, 1200/1201 — см.
 * docs/design-tokens/03-breakpoints.md, раздел «Четвёртая пара»).
 * `.calendar-layout` в Клиндарии схлопывается на `max-width:1180px`;
 * `.nv-row` в том же файле должен учитывать правую detail-панель ровно там,
 * где она реально появляется — на `min-width:1181px`, а не на уже
 * существующем `1201px` (это создало бы разрыв геометрии на 1181–1200px).
 */
const LEGACY = [420, 560, 680, 700, 720, 760, 767, 768, 820, 860,
                1000, 1100, 1101, 1180, 1181, 1201, 1240, 1280];

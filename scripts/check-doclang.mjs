...
console.log('\n4. Шрифт и санитайзер PDF');
{
  ...
  // прежний порог 150 пришлось опустить. 14.08.2026 — второй такой заход:
  // подписи полей, повторявшиеся в анкете, карточке, выгрузке и сводке,
  // сведены в doc.ps.field.*, строки про отправку — в doc.ps.send.*, словарь
  // стал 110 строк, порог опущен со 120 до 95. Опускать дальше без такой же
  // явной причины нельзя: смысл порога в том, чтобы падать, если разбор
  // сломался, а не подстраиваться под текущее число.
  ok('строк словаря найдено', strings.length > 95, String(strings.length));
  const bad = new Set();

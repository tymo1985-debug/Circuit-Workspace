...
/* --- 3б. Снятие копии не создаёт призрачных баз (N-9) --------------------
 *
 * ПОЧЕМУ ЭТО ПРОВЕРЯЕТСЯ. `indexedDB.open(name)` без версии базу СОЗДАЁТ,
 * если её нет. До 29.08.2026 снятие копии хаба на устройстве, где какой-то
 * модуль ни разу не открывали, оставляло после себя пустую базу этого модуля.
 * Сама по себе она безвредна — но она же неотличима от настоящей ничем, кроме
 * отсутствия хранилищ, а именно на этом различии держится потолок версии для
 * баз модулей (сценарий 5.6). Меньше призраков — меньше поводов ошибиться.
 */
console.log('\nПризрачные базы');
const GHOST = 'pioneer-school-db';
await wipe(GHOST);
const before9 = (await indexedDB.databases()).map((d) => d.name);
ok('подготовка: базы модуля на устройстве нет', before9.indexOf(GHOST) < 0, before9.join(', '));
await CWBackup.snapshot();
const after9 = (await indexedDB.databases()).map((d) => d.name);
ok('снятие копии не создало базу отсутствующего модуля', after9.indexOf(GHOST) < 0,
  'после snapshot() появилась пустая ' + GHOST + ' — призрак, неотличимый от настоящей базы '
  + 'ничем, кроме отсутствия хранилищ');
...
/* 5.6. НАСТОЯЩАЯ база модуля и копия из БОЛЕЕ НОВОЙ версии этого модуля
   (лишнее хранилище). Отличие от 5.2 принципиальное: там база «призрачная»
   (ноль хранилищ, создана openExisting() на устройстве, где модуль ни разу не
   открывали) и поднять её версию законно; здесь база настоящая, со своими
   хранилищами, и подъём означает VersionError для модуля НАВСЕГДА.

   Отличить одно от другого можно без знания константы модуля: у призрачной
   базы хранилищ нет вообще, у настоящей — есть. Именно это и проверяется.
   Сценарий добавлен 29.08.2026 по итогам повторного аудита (находка N-1);
   на коде до правки он падал: потолком для базы модуля служила схема самой
   копии, то есть ровно та величина, до которой база и поднималась. */
await wipe(SCHOOL);
d = await open(SCHOOL, 2, (x) => {
  x.createObjectStore('students', { keyPath: 'id' });
  x.createObjectStore('classes', { keyPath: 'id' });
});
await put(d, 'students', [{ id: 's_keep', name: 'Учащийся до восстановления' }]);
d.close();
mem.set('cw-sender', JSON.stringify({ name: 'ДоВосстановленияШколы' }));
let schoolRefusal = null;
const schoolFile = wrap('pioneer-school', SCHOOL, {
  version: 3,
  stores: {
    students: store([{ id: 's_from_copy', name: 'Из копии' }]),
    /* Хранилище появилось в более новой версии Школы. Создать его можно
       только подняв базу до 3, а pioneer-school/js/db.js открывает её 2. */
    mentors: store([{ id: 'm1' }]),
  },
});
schoolFile.sections.shared = { local: { 'cw-sender': JSON.stringify({ name: 'ИзКопии' }) } };
try {
  await CWBackup.restore(schoolFile);
} catch (e) { schoolRefusal = e && e.message; }
ok('база модуля: копия из более новой версии модуля отклонена',
  schoolRefusal === 'backup-newer-schema',
  schoolRefusal === null ? 'восстановление прошло молча — база модуля поднята выше его DB_VERSION' : schoolRefusal);
ok('база модуля: версия после отказа не изменилась', await versionOf(SCHOOL) === 2,
  'поднятая версия необратима: понизить IndexedDB нельзя');
ok('база модуля: модуль по-прежнему открывает свою базу', await opensAt(SCHOOL, 2) === 'открылась',
  'ровно этот VersionError и означал бы потерю доступа к учащимся, классам и регистрациям');
ok('база модуля: отклонённое восстановление не тронуло данные',
  (await rows(SCHOOL, 'students')).some((r) => r.id === 's_keep'));
/* Ровно то, что дал бы симметричный отказ в inspect(): к моменту отказа на
   устройстве не изменено НИЧЕГО. inspect() базы модулей проверить не может —
   их схема известна только после открытия базы, то есть асинхронно (см.
   комментарий в shared/backup.js). Отказ ловит фаза A, и цена его такая же. */
ok('база модуля: отказ наступил до первой записи (localStorage цел)',
  JSON.parse(mem.get('cw-sender')).name === 'ДоВосстановленияШколы',
  'фаза A обязана отказывать раньше, чем фаза C пишет localStorage');

/* 5.7. Регрессия к 5.6: настоящая база модуля и копия с ТЕМ ЖЕ набором
   хранилищ восстанавливается штатно. Без этого утверждения правку можно было
   бы «починить» запретом на любое восстановление баз модулей. */
await wipe(SCHOOL);
d = await open(SCHOOL, 2, (x) => { x.createObjectStore('students', { keyPath: 'id' }); });
d.close();
await CWBackup.restore(wrap('pioneer-school', SCHOOL, {
  version: 2,
  stores: { students: store([{ id: 's_plain', name: 'Штатный путь' }]) },
}));
ok('база модуля: штатное восстановление проходит', (await rows(SCHOOL, 'students')).some((r) => r.id === 's_plain'));
ok('база модуля: штатное восстановление не подняло версию', await versionOf(SCHOOL) === 2);

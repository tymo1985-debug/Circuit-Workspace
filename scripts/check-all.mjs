...
const CHECKS = [
  ...
  {
    file: 'check-hub-order.mjs',
    title: 'Порядок плиток хаба',
    why: 'сохранённый порядок переживает выпуски: ошибка нормализации молча переносит плитку в чужой раздел, дублирует её или теряет новый модуль',
    deps: ['jsdom'],
  },
  {
  ...
  {
    file: 'check-journal-overview.mjs',
    title: 'Данные живого Обзора Журнала (O1)',
    why: 'счётчик объектов не должен считать посещения вместо объектов и путать «не прочитано» с нулём; сводка Обзора — только чтение с тем же архивным контекстом и порядком, что рабочие экраны, без раскрытия защищённого текста и без собственного хранилища',
    deps: ['fake-indexeddb'],
  },
  {

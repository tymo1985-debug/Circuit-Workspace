...
(function () {
  ...
  function resolveCommunity(id) { return directoryReady && id ? CWDirectory.get(id) : null; }
  ...
  function searchRow(r, query) {
    ...
    var go = function () {
      ...
      if (href === '#tasks') taskTab = r.row.status === 'done' ? 'done' : 'open';
      location.hash = href;
    };
  ...
  /* ═══ Архив (J6) ═════════════════════════════════════════════════════════
   * CWJournal.archive: только напрямую архивные узлы и посещения, новые
  ...
  async function renderArchive() {
    ...
    var list = items.filter(function (it) {
      return archiveTab === 'all' || (archiveTab === 'nodes' ? it.kind === 'node' : it.kind === 'visit');
    });
    ...
    list.forEach(function (it) {
      ...
      ico.innerHTML = svg(it.kind === 'visit' ? ICON.visit : (ICON[it.row.kind] || ICON.group));
  ...
  function applyRoute() {
    ...
    if (state.route === 'districts') {
      ...
      if (state.route === 'tasks') renderTasks();
        ...
        renderSearch();
        if (searchUi.focusNext) { searchUi.focusNext = false; focusSearch(); }

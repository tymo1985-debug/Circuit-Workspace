...
(function (global) {
  ...
  global.CW_TEMPLATE_NAMESPACES = {
    ...
    /* Школа пионеров. Пространства были заведены заранее (11.08.2026) с тремя
       полями-заготовками; 14.08.2026, вместе с первым письмом модуля (фаза 6),
       заполнены реальным составом.

       АЛИАСОВ ЗДЕСЬ НЕТ И НЕ БУДЕТ ЗАДНИМ ЧИСЛОМ. У пользователей нет ни
       одного старого шаблона Школы — писем в модуле не существовало вовсе,
       переименовывать нечего. Но обратная сторона того же правила: имена,
       выпущенные сейчас, менять уже нельзя — как только кто-то отредактирует
       шаблон, они окажутся написаны у него в браузере. */
    student: {
      fields: {
        firstName:    { aliases: [], example: 'Олена' },
        lastName:     { aliases: [], example: 'Коваль' },
        /* Строка, введённая в карточке учащегося, а не ссылка на справочник:
           общего справочника собраний (shared/directory.js) ещё нет. Когда он
           появится, источник значения сменится, а имя переменной — нет. */
        congregation: { aliases: [], example: 'Варшава-Бемово' },
    ...
    school: {
      fields: {
        startDate:              { aliases: [], example: '12 жовтня 2026 р.' },
        endDate:                { aliases: [], example: '17 жовтня 2026 р.' },
        place:                  { aliases: [], example: 'Зал Царства, Варшава-Бемово' },
        teacherA:               { aliases: [], example: 'Олексій Тимощук' },
        teacherB:               { aliases: [], example: 'Якуб Ульфік' },
        /* Срок и контакты сдачи анкеты. Живут в настройках формуляра
           (meta.registrationConfig), а не в «Назначении»: их задаёт тот же
           человек и в том же месте, где рассылает ссылку на анкету. */
        registrationDeadline:   { aliases: [], example: '20 вересня 2026 р.' },
        registrationEmail:      { aliases: [], example: 'school@example.com' },
        registrationWhatsapp:   { aliases: [], example: '+48 000 000 000' },
      },
    },

    /* Проект района — Журнал (J9b, 24.09.2026). Только два поля: название и
       описание проекта, как их видит человек. Защищённый проект (J8) письма
       не получает вовсе — открытый текст не уходит в архив документов. */
    project: {
      fields: {
        title: { aliases: [], example: 'Подготовка специальной встречи' },
        body:  { aliases: [], example: 'Описание проекта' },
      },
    },
  };

...
(function (global) {
  ...
  var MODULES = {
    ...
    /* Журнал (фаза J2). Четыре хранилища принадлежат Журналу целиком, поэтому
       выгружаются полностью, без адресного отбора по ключу (в отличие от
       `state`/`snapshots`, где рядом лежат строки соседей).
       `communities` — по той же причине, что у Клиндария: узлы собраний
       ссылаются на карточки справочника по `communityId`, и копия, поднятая на
       чистом устройстве без них, дала бы висячие ссылки — бесшумно.
       Восстановление копии модуля — слияние по ключу (partial): чужие
       хранилища и строки соседей не трогаются.
       ИЗВЕСТНАЯ АСИММЕТРИЯ (не дефект этой записи): полная копия хаба, снятая
       ДО появления Журнала, этих хранилищ не содержит, и её восстановление
       оставляет текущие данные Журнала нетронутыми — writeDb очищает только
       хранилища, присутствующие в файле.
       J9b (24.09.2026): письма проекта — `templates` (правка шаблона
       `sys.journal.project.letter`) и `documents` (снимки выданных писем).
       Как у остальных модулей — хранилища целиком и СЛИЯНИЕМ: в набор замены
       J8 они не входят, копия Журнала чужих шаблонов и снимков не стирает. */
    'journal': {
      local: [],
      idb: [],
      sharedLocal: [],
      sharedStores: { 'circuit-workspace-db': ['journalNodes', 'journalEntries', 'journalLinks', 'journalMeta', 'communities', 'templates', 'documents'] },

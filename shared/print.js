...
(function (global) {
  ...
  /** `@page` собирается только если его попросили: без правила действуют
   *  поля из настроек печати браузера — текущее поведение обоих композеров. */
  function pageRule(page) {
    if (!page) return '';
    var parts = [];
    if (page.size) parts.push('size:' + page.size);
    if (page.margin) parts.push('margin:' + page.margin);
    return parts.length ? '@media print{@page{' + parts.join(';') + '}}' : '';
  }

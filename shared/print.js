...
(function (global) {
  ...
  /* Имя оставлено прежним: оно читается в десятке мест этого файла, а
     переименование ради переименования — лишний диф в чувствительном
     тракте печати. Апостроф теперь экранируется тоже: раньше эта редакция
     была одной из трёх слабых. */
  function escapeText(s) {
    return global.CWEscape.html(s);
  }
  ...
  function printWhenReady(win) {
    ...
    function fire() {
      ...
      /* Закрытие ПОСЛЕ печати, и подписка ставится до самого вызова: в части
         движков `print()` блокирующий, и обработчик, повешенный после него,
         на своё событие уже не успеет.
         `afterprint` наступает и когда пользователь отменил диалог — это
         правильно: висящая пустая вкладка не нужна ни при печати, ни при
         отказе от неё. Окно открыто нами, поэтому close() разрешён.
         Движок без `afterprint` просто оставит окно — прежнее поведение, а не
         отказ. */
      try {
        win.addEventListener('afterprint', function () {
          try { win.close(); } catch (_) {}
        });
      } catch (_) { /* окно уже закрыто пользователем */ }

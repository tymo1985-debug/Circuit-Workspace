...
import { clean, dt, esc, fmt, id, isSection, noAssignmentNeeded, tableHeading } from "./utils.js";
...
по-прежнему меняет ту же модель. */let mn=$("#congressMetaName");if(mn)mn.textContent=c.name;let ml=$("#congressMetaLine");if(ml){let parts=[];if(c.date)parts.push("📅 "+fmt(c.date));if(c.place)parts.push("📍 "+esc(c.place));ml.innerHTML=parts.join(" · ")}renderTasks()
  /* Блокировка доменных контролов ставится ПОСЛЕ перерисовки: строки заданий
     пересоздаются через innerHTML, и без повторного вызова новые элементы
     остались бы редактируемыми, а UI выглядел бы рабочим. */
  applyDegradedUI();}
export function renderCongresses(){let b=$("#congressList");let series=store.st.series||[];let bySeries={},none=[];store.st.congresses.forEach(c=>{if(c.seriesId&&series.some(s=>s.id===c.seriesId))(bySeries[c.seriesId]=bySeries[c.seriesId]||[]).push(c);else none.push(c)});let cardHTML=c=>`<div class="congress ${c.id===store.st.activeId?"active":""}" data-c="${c.id}"><b>${esc(c.name)}</b><br><small>${esc(c.date||"")}</small></div>`;let html="";series.forEach(s=>{let items=bySeries[s.id]||[];html+=`<div class="series-header"><span>${esc(s.name)}</span><span class="series-actions"><button type="button" class="tiny light icon-text-btn" data-addc="${s.id}" title="${esc(t("cong.title.add_to_series"))}">${icon("plus")}<span>${esc(t("cong.btn.add_congress"))}</span></button>${items.length>1?`<button type="button" class="tiny light icon-btn" data-linkseries="${s.id}" title="${esc(t("cong.title.link_series"))}" aria-label="${esc(t("cong.title.link_series"))}">${icon("link")}</button>`:""}<button type="button" class="tiny danger icon-btn" data-delseries="${s.id}" title="${esc(t("cong.btn.delete_series"))}" aria-label="${esc(t("cong.btn.delete_series"))}">${icon("trash")}</button></span></div>`;html+=items.length?items.map(cardHTML).join(""):`<p class="hint series-empty">${esc(t("cong.hint.series_empty"))}</p>`});if(none.length){if(series.length)html+=`<div class="series-header"><span>${esc(t("cong.opt.no_series"))}</span></div>`;html+=none.map(cardHTML).join("")}b.innerHTML=html;$$("#congressList .congress").forEach(d=>d.onclick=()=>{store.st.activeId=d.dataset.c;let c=A();store.sel=c?.tasks[0]?.id||null;save();render()});$$("#congressList [data-addc]").forEach(btn=>btn.onclick=()=>openNew(btn.dataset.addc));$$("#congressList [data-linkseries]").forEach(btn=>btn.onclick=()=>openLinkSeries(btn.dataset.linkseries));$$("#congressList [data-delseries]").forEach(btn=>btn.onclick=()=>{if(!confirm(t("cong.confirm.delete_series")))return;let sid=btn.dataset.delseries;store.st.congresses.forEach(c=>{if(c.seriesId===sid)c.seriesId=null});store.st.series=store.st.series.filter(s=>s.id!==sid);save();renderCongresses()})
  /* Частичная перерисовка пересоздаёт динамические доменные контролы
     (.status-sel, .rm, .ed, .le, [data-addc], [data-delseries],
     [data-linkseries]). Вызывается отдельно от render(), поэтому блокировку
...
булево поле (js/state.js `row()`), которое может разойтись со статусом
(ручная смена статуса в селекте не трогает letterSent). Отдельная колонка
убрана из десктоп-таблицы, но сам факт остаётся виден — компактной иконкой
рядом со status-select, а не вторым badge. Показывается только когда письмо
вообще применимо к строке (noAssignmentNeeded=false), иначе — ничего. */
function mailIndicatorHTML(t){if(noAssignmentNeeded(t))return"";return t.letterSent?`<span class="mail-flag mail-flag--sent" title="${esc(tr_("cong.status.letter_sent"))}" aria-label="${esc(tr_("cong.status.letter_sent"))}">${icon("mail")}</span>`:`<span class="mail-flag mail-flag--pending" title="${esc(tr_("cong.status.letter_pending"))}" aria-label="${esc(tr_("cong.status.letter_pending"))}">${icon("mail")}</span>`}
export function renderTasks(){let c=A(),b=$("#tasksBody");if(!b)return;b.innerHTML="";if(!c)return;(c.tasks||[]).forEach(t=>{let section=isSection(t);let statusSel=noAssignmentNeeded(t)?"":`<select class="status status-sel" title="${esc(tr_("cong.title.task_status"))}" aria-label="${esc(tr_("cong.title.task_status"))}" data-s="${esc(t.status||"")}">${STATUSES.map(s=>`<option value="${esc(s)}"${s===(t.status||"")?" selected":""}>${esc(tStatus(s))}</option>`).join("")}</select>`;let letterBtn=noAssignmentNeeded(t)?"":`<button type="button" class="tiny icon-btn le" title="${esc(tr_("cong.th.letter"))}" aria-label="${esc(tr_("cong.th.letter"))}">${icon("mail")}</button>`;let linkMark=t.linkId?`<span class="link-badge" title="${esc(tr_("cong.link.badge_title"))}">${icon("link")}${linkCount(t.linkId)}</span>`:"";let tr=document.createElement("tr");tr.className=(section?"section-title ":"")+(t.id===store.sel?"selected":"");tr.onclick=e=>{if(e.target.closest("button,select,input,textarea,a,label"))return;let compact=window.matchMedia("(max-width:680px)").matches;if(compact){openEdit(t.id);return}store.sel=t.id;renderTasks()};tr.innerHTML=`<td class="time" data-label="${esc(tr_("cong.th.time"))}">${esc(dt(t.time))}</td><td class="num" data-label="№">${esc(t.number)}</td><td class="topic" data-label="${esc(tr_("cong.th.topic"))}">${esc(t.title)}${linkMark}</td><td class="speaker" data-label="${esc(tr_("cong.th.speaker"))}">${peopleHTML(t)}</td><td class="kind" data-label="${esc(tr_("cong.th.interview"))}">${t.kind?`<span class="pill">${esc(t.kind)}</span>`:""}</td><td class="dur" data-label="${esc(tr_("cong.th.min"))}">${esc(t.duration)}</td><td class="status-cell" data-label="${esc(tr_("cong.th.status"))}"><div class="status-cell__row">${statusSel}${mailIndicatorHTML(t)}</div></td><td class="reh" data-label="${esc(tr_("cong.th.rehearsal"))}">${t.rehearsal?"✓":""}</td><td class="actions-cell" data-label="${esc(tr_("cong.th.actions"))}"><div class="row-actions"><button type="button" class="tiny icon-btn ed" title="${esc(tr_("cong.btn.edit"))}" aria-label="${esc(tr_("cong.btn.edit"))}">${icon("edit")}</button>${letterBtn}<div class="md-menu row-menu"><button type="button" class="tiny icon-btn row-more" title="${esc(tr_("cong.title.more"))}" aria-label="${esc(tr_("cong.title.more"))}" aria-haspopup="true" aria-expanded="false">${icon("more")}</button><div class="md-menu__panel md-menu__panel-end" role="menu" hidden><button type="button" class="md-menu__item md-menu__item-danger rm" role="menuitem">${icon("trash")}<span>${esc(tr_("cong.btn.delete"))}</span></button></div></div></div></td>`;b.appendChild(tr);tr.querySelector('.ed').onclick=e=>{e.stopPropagation();openEdit(t.id)};let le=tr.querySelector('.le');if(le)le.onclick=e=>{e.stopPropagation();openLetter(t.id)};tr.querySelector('.rm').onclick=e=>{e.stopPropagation();removeTask(t.id)};let ss=tr.querySelector('.status-sel');if(ss)ss.onchange=e=>{e.stopPropagation();t.status=ss.value;ss.dataset.s=ss.value;propagateStatus(t);save()}});bindRowMenus(b)}
/* Меню «⋮» построчно (delete в overflow). Один делегирующий обработчик на
tbody вместо N слушателей на N кнопок — renderTasks() перерисовывает
содержимое таблицы (innerHTML), но сам узел #tasksBody не пересоздаётся,
поэтому обработчик вешается один раз и переживает все последующие
перерисовки (иначе addEventListener копился бы с каждым render). Поведение
(открыть/закрыть/Esc/клик мимо) сознательно не через initMenu() из
topbar-menu.js: та фабрика на один статичный триггер+панель, а здесь их
много и они появляются/исчезают вместе со строками. */
let rowMenusBound=false;
function closeAllRowMenus(except){$$(".row-menu .md-menu__panel").forEach(p=>{if(p===except)return;if(p.hidden)return;p.hidden=true;let btn=p.previousElementSibling;if(btn)btn.setAttribute("aria-expanded","false")})}
function bindRowMenus(tbody){if(rowMenusBound)return;rowMenusBound=true;tbody.addEventListener("click",e=>{let trigger=e.target.closest(".row-more");if(trigger){e.stopPropagation();let panel=trigger.nextElementSibling;let willOpen=panel.hidden;closeAllRowMenus();panel.hidden=!willOpen;trigger.setAttribute("aria-expanded",willOpen?"true":"false");return}if(e.target.closest(".row-menu .md-menu__panel"))closeAllRowMenus()});document.addEventListener("click",e=>{if(e.target.closest(".row-menu"))return;closeAllRowMenus()});document.addEventListener("keydown",e=>{if(e.key!=="Escape")return;let open=document.querySelector(".row-menu .md-menu__panel:not([hidden])");if(!open)return;e.stopPropagation();let btn=open.previousElementSibling;closeAllRowMenus();if(btn){btn.setAttribute("aria-expanded","false");btn.focus()}})
  /* Частичная перерисовка пересоздаёт динамические доменные контролы
     (.status-sel, .rm, .ed, .le, [data-addc], [data-delseries],
     [data-linkseries]). Вызывается отдельно от render(), поэтому блокировку
     нужно поставить здесь заново — иначе контролы снова кликабельны. */

...
export function saveTemplate(){let s=S();if(store.templateMode==="type"){let type=$("#templateTypeName").value.trim();if(!type)return alert(t("cong.alert.type_required"));s.templatesByType[type]=$("#templateEditor").value}else{s.templates[s.language]=$("#templateEditor").value}save();alert(t("cong.msg.template_saved"))}
export function resetTemplate(){if(!confirm(t("cong.confirm.reset_template")))return;let s=S();if(store.templateMode==="type"){delete s.templatesByType[$("#templateTypeName").value.trim()];$("#templateEditor").value=s.templates[s.language]||DEFAULT_TEMPLATES[s.language]}else{s.templates[s.language]=DEFAULT_TEMPLATES[s.language];$("#templateEditor").value=s.templates[s.language]}save()}

...
import { allLetters, copyComposerText, openLetter, openLettersMode, printLetter, saveComposerToArchive, toggleComposerEdit } from "./letters.js";
...
makeBackup(t("cong.msg.before_reset")).then(ok=>{if(!ok){alert(t("cong.alert.backup_failed"));return}
...
initState().then(()=>{load();initMobile()}).catch(e=>{let b=$("#errorBox");if(b){b.textContent="Ошибка запуска: "+e.message+"\n"+e.stack;b.classList.remove("hidden")}});

...
export function drawParts(ps){let b=$("#editParticipants");b.innerHTML="";ps.forEach((p,i)=>{let d=document.createElement("div");d.className="epart";d.innerHTML=`<input list="speakerDatalist" value="${esc(p.name)}" placeholder="${esc(t("cong.ph.participant_name"))}"><input list="congregationDatalist" value="${esc(p.congregation)}" placeholder="Собрание / группа"><input type="tel" inputmode="tel" value="${esc(p.phone||"")}" placeholder="${esc(t("cong.ph.participant_phone"))}"><button type="button" class="danger icon-btn" title="Удалить участника" aria-label="Удалить участника">${icon("trash")}</button>`;let nameInput=d.children[0],congInput=d.children[1];nameInput.onchange=()=>{let prof=(S().speakerProfiles||[]).find(x=>x.name===nameInput.value);if(prof&&!congInput.value)congInput.value=prof.congregation||""};d.querySelector("button").onclick=()=>{ps.splice(i,1);drawParts(ps)};b.appendChild(d)})}

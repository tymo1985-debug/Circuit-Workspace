...
export function openList(mode){store.listMode=mode;let s=S(),map={groups:["congregations",t("cong.btn.groups"),"group"],speakers:["speakers",t("cong.btn.speakers"),"speaker"],types:["assignmentTypes",t("cong.field.type"),"type"],kinds:["assignmentKinds",t("cong.field.interview_show"),"kind"]}[mode];$("#listTitle").textContent=map[1];$("#listHint").textContent=t("cong.hint.list_values");/* Сверка со справочником имеет смысл только для собраний: докладчиков,
   типов и видов заданий общий справочник не знает. */{let b=$("#matchDirectoryBtn");if(b)b.hidden=mode!=="groups"}$("#listEditor").value=clean(s[map[0]]).join("\n");renderListPreview();$("#listDialog").showModal()}

...
(function () {
  ...
  function setView(view) {
    ...
    /* Кнопка вставки переменной и picker относятся ИСКЛЮЧИТЕЛЬНО к вкладке
       «Текст» — источник истины здесь `view === 'edit'` напрямую, а не
       видимость `#viewEdit`. Раньше было `$('#insertVarBtn').hidden =
       $('#viewEdit').hidden` — ошибка: `#viewEdit.hidden` описывает РАСКЛАДКУ
       (что показано рядом с чем на экране), а не то, разрешено ли редактирование.
       Split-view держит #viewEdit видимым одновременно с #viewPreview именно
       на вкладке «Текст» — это единственный случай, где оба совпадают. Но
       смешивать эти два понятия хрупко: split не должен превращать Preview

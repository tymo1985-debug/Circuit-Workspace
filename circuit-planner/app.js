    ...
    ui: {
      cacheElements() {
        [
          ...
          'languageSelect','themeSelect','fontSizeSelect',
...
document.querySelectorAll('.sy-day[data-add-date]').forEach((btn) => {
 ...
 document.addEventListener('click', (e) => { const popover = document.getElementById('dayPopover'); if (!popover || popover.hidden) return; if (popover.contains(e.target) || e.target.closest('.sy-day')) return; App.ui.hideDayPopover(true); });
  ...
  // Части, зарегистрированные в window.CPParts (см. circuit-planner/ui/*.js),
  // подмешиваются здесь — после того как App полностью собран, и до того как
  // что-либо (включая App.init() ниже) начинает выполняться. Порядок
  // подключения <script> у самих частей безразличен; обязательно только то,
  // что они подключены раньше app.js.
  // Второй аргумент — константы формуляра посещения/писем, объявленные в
  // строках 21-136 (VP_I18N_DICTS и соседние). Они передаются частям
  // напрямую, а не переносятся на App: App остаётся ровно тем же объектом,
  // что и раньше, без нового публичного свойства. Решение принято 31.08.2026
  // при подготовке среза «письма» — предпочтено App.const как более локальное:
  // ни одна декларация не переезжает, порядок инициализации не меняется,
  // существующий код в этом же файле продолжает видеть те же самые
  // переменные напрямую, как и раньше.
  var CPConsts = {
    VP_I18N_DICTS, VP_LANG_NAMES, buildVpI18n, vpEscapeForHtml,
    DEFAULT_LETTER_TEMPLATE_HTML, DEFAULT_EMAIL_BODY_TEMPLATES,
    DEFAULT_LETTER_SALUTATIONS, LETTER_SUBJECT_PARTS, builtinPages,
  };
  (window.CPParts || []).forEach(function (part) { part(App, CPConsts); });
